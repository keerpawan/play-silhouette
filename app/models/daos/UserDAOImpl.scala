...
class UserDAOImpl @Inject() (protected val dbConfigProvider: DatabaseConfigProvider) extends UserDAO with DAOSlick {
  ...
  def verify(token: Long, email: String) = {
    val userQuery = for {
      result <- slickUsers.filter(x => x.email === email && x.activated === false).map(_.activated).update(true)
      dbUser <- slickUsers.filter(x => x.email === email && x.activated === true).result.headOption
    } yield (result, dbUser)
    db.run(userQuery).map {
      case (_, Some(user: DBUser)) => true
      case _ => false
    }
  }
